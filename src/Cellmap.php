...
class Cellmap
{
     ...
     * parts or, if there are none, to the percentage rows in proportion to
     * their percentages, as browsers do, or else to all the rows in
     * proportion to their height. The row groups and rows already laid out below the ones
     * that grow are moved down accordingly, along with their cells.
     *
     ...
     */
    public function distribute_height(float $extra): array
    {
        if ($extra <= 0) {
            return [];
        }
        ...
        foreach ($this->_frames as $arr) {
            ...
            if ($display === "table-row" && !in_array($index, $indexes, true)) {
                $indexes[] = $index;
        ...
        // The remainder goes to the auto rows in equal parts. Without auto
        // rows the specification gives it to every row in equal parts, while
        // browsers let the percentage rows grow in proportion to their
        // percentages (see the investigation in css-tables-3 §3.10.4); follow
        // them, and grow the rows in proportion to their height as a last
        // resort
        $remaining = $extra - $granted;
        $auto = array_diff($indexes, array_keys($percentages), array_keys($fixed));
        $total = array_sum($percentages);
        $content = $reference - $extra;

        foreach ($indexes as $index) {
            if ($auto !== []) {
                $share = in_array($index, $auto, true) ? 1 / count($auto) : 0;
            } elseif ($total > 0) {
                $share = ($percentages[$index] ?? 0) / $total;
            } elseif ($content > 0) {
                $share = ($this->_rows[$index]["height"] ?? 0) / $content;
            } else {
                $share = 1 / count($indexes);
            }
