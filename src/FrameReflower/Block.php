...
class Block extends AbstractFrameReflower
{
    ...
    protected function reflow_orthogonal(?BlockFrameDecorator $block): void
    {
        ...
        $inline_size = $this->resolve_height($cb["h"]);

        if ($inline_size === "auto") {
            [$min, $max] = $this->get_min_max_child_width();
            $inline_size = max($min, min($max, $cb["h"] ?? (float) $frame->get_root()->get_containing_block("h")));
        }
        ...
        $style->set_used("top", $style->length_in_pt($style->top, $cb["h"] ?? 0));
        $style->set_used("bottom", $style->length_in_pt($style->bottom, $cb["h"] ?? 0));
